  ...
  return (
    <div>
      ...
      <section className="mt-6">
        ...
        <ExpenseList groupId={groupId} expenses={expenses || []} memberMap={memberMap} currency={group.currency} onAdd={() => setShowAdd(true)} />
      </section>

      <InviteDialog open={showInvite} onClose={() => setShowInvite(false)} groupId={groupId} groupName={group.name} />
      <AddExpenseDialog open={showAdd} onClose={() => setShowAdd(false)} groupId={groupId} members={group.members} currency={group.currency} />
    </div>
  )
}