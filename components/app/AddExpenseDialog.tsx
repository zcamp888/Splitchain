...
export function AddExpenseDialog({
  ...
  useEffect(() => {
    ...
    setSplitMode('equal')
    setExactSplits({})
    setIncludedMembers(new Set(members.map((m) => m.user_id)))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let splits: { user_id: string; share_amount: number; share_type: 'equal' | 'exact' }[]
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      <div className="glass-strong max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl p-6 shadow-2xl sm:rounded-3xl" onClick={(e) => e.stopPropagation()} style={{ overscrollBehavior: 'contain' }}>
        <div className="flex items-start justify-between">
          <h2 id="add-expense-title" className="font-display text-xl font-bold tracking-tight">Add expense</h2>
                ...
                {members.map((m) => {
                  ...
                  return (
                    <option key={m.user_id} value={m.user_id}>
                      {m.user_id === currentUserId ? `You (${label})` : label}
                    </option>
                  )
                })}
              </select>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <div className="grid grid-cols-2 gap-1 rounded-lg border border-border-strong bg-bg-elev/40 p-0.5 text-xs">
                <button type="button" onClick={() => setSplitMode('equal')} className={`rounded-md px-2.5 py-1 font-medium transition-colors ${splitMode === 'equal' ? 'bg-bg-card' : 'text-fg-muted hover:text-fg'}`}>Equal</button>
                <button type="button" onClick={() => setSplitMode('exact')} className={`rounded-md px-2.5 py-1 font-medium transition-colors ${splitMode === 'exact' ? 'bg-bg-card' : 'text-fg-muted hover:text-fg'}`}>Exact</button>
              </div>
            </div>
            <div className="space-y-1.5 rounded-xl border border-border-strong bg-bg-elev/40 p-2">
              {members.map((m) => {
                ...
                return (
                  <label key={m.user_id} htmlFor={`split-${m.user_id}`} className="flex cursor-pointer items-center gap-3 rounded-lg px-2.5 py-2 transition-colors hover:bg-bg-card/60">
                    <input id={`split-${m.user_id}`} type="checkbox" checked={checked} onChange={() => toggleMember(m.user_id)} className="h-4 w-4 shrink-0 accent-neon-violet" />
                    <span className="min-w-0 flex-1 truncate text-sm">{label}</span>
                    {checked && splitMode === 'equal' && <span className="tabular font-mono text-xs text-fg-muted">{equalShare.toFixed(2)}</span>}
