...
export function AuthPanel() {
  ...
  const postAuthDestination = inviteToken ? `/invite/${inviteToken}` : '/app'
  ...
  const handleWalletAuth = async () => {
    ...
    try {
      ...
      push({ kind: 'success', message: inviteToken ? 'Signed in — accepting invite…' : 'Welcome back' })
  ...
  return (
    <div className="glass-strong rounded-3xl p-8 shadow-2xl">
      ...
      <p className="mt-2 text-sm text-fg-muted">
        {inviteToken ? 'Sign in to accept your invite.' : 'Connect your wallet or use email.'}
      </p>
      ...
      {mode === 'wallet' ? (
        ...
        <form onSubmit={handleEmailAuth} className="mt-6 space-y-3">
          <div>
            <label htmlFor="email" className="mb-1.5 block text-xs text-fg-muted">
              Email address
            </label>
            <input
              id="email"
