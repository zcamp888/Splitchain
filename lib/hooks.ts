  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('group_members')
        .select(
          `role, joined_at, groups:group_id (id, name, description, currency, cover_emoji, created_at, created_by)`
        )
        .eq('user_id', user.id)
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: members } = await supabase
        .from('group_members')
        .select(
          `role, joined_at, user_id, profiles:user_id (id, display_name, email, wallet_address)`
        )
        .eq('group_id', groupId)
  ...
  return useMutation({
    mutationFn: async (input: {
      ...
      if (Math.abs(splitsTotal - input.amount) > 0.01) {
        throw new Error(
          `Splits (${splitsTotal.toFixed(2)}) must equal amount (${input.amount.toFixed(2)})`
        )
      }
