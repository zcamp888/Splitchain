  ...
  return useMutation({
    mutationFn: async ({ groupId, groupName }: { groupId: string; groupName: string }) => {
      ...
      const [{ data: group }, { data: members }, { data: expenses }, { data: settlements }] = await Promise.all([
        ...
        supabase
          .from('group_members')
          .select('user_id, profiles:user_id(id, nickname, display_name, email, wallet_address)')
          .eq('group_id', groupId),
        supabase
          .from('expenses')
      ...
      const expenseRows = (expenses || []).map((e: any) => ({
        ...
        currency: e.currency,
        paid_by: displayName(profileMap.get(e.paid_by)),
        split_count: (e.splits || []).length,
        split_detail: (e.splits || [])
          .map((s: any) => `${displayName(profileMap.get(s.user_id))}: ${Number(s.share_amount).toFixed(2)}`)
          .join('; '),
      }))

      const settlementRows = (settlements || []).map((s: any) => ({
        date: (s.confirmed_at || s.created_at).slice(0, 10),
        from: displayName(profileMap.get(s.from_user)),
        to: displayName(profileMap.get(s.to_user)),
      ...
      const balanceRows = balances.map((b) => ({
        member: displayName(profileMap.get(b.user_id)),
        net: b.net.toFixed(2),
